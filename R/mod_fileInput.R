...
observeEvent(input$loadCode, {
  ...
  # Observe and load fold-change CSV
  observeEvent(input$loadFC, {
    r$rendering <- TRUE
    r$fileInput <- readFile(input$inputFC$datapath)
    r$stage$fileColor <- r$fileInput$stage$backgroundColor
    r$fileInput$name <- tools::file_path_sans_ext(input$inputPDB$name)
  })
}
