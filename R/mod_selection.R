...
mod_selection_server <- function(input, output, session, globalSession, r) {
  ...
  observeEvent(r$sidebarItemExpanded, {
    if (r$selection$loaded == FALSE && (r$sidebarItemExpanded == "selection")) { # reset in mod_NGLVieweROutput
      ...
      updateSelectInput(session, "selectionType", selected = "surface")
  ...
  #   Viewer_proxy %>% removeSelection("selection")
    ...
    # } else {
    #   # --- Check if selection is a gene name ---
    #   if (!is.null(r$gene_map) && input$selection %in% r$gene_map$genename) {
    #     chains <- r$gene_map$mappings.chain_id[r$gene_map$genename == input$selection]
    #     selection <- chains[!is.na(chains)]
    #
    #   } else {
    #     # Fall back to normal NGL/residue selection parsing
    #     selection <- selection_to_ngl(
    #       paste0(r$sequence_df$AA, collapse = ""),
    #       input$selection
    #     )
    #   }
    # }
    ...
    #   selection <- "none"
    # } else {
    #   selection <- selection_to_ngl(paste0(r$sequence_df$AA, collapse = ""), isolate(input$selection), isolate(r$gene_map))
    # }
